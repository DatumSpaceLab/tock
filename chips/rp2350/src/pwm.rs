...
// RP2350 port (DatumSpace K.K., 2026): 12 slices, EN/INTR moved, IRQ0/IRQ1 interrupt registers, GPIO 16-31 on slices 8-11.
...
register_structs! {
    PwmRegisters {
        ...
        // IRQ0_INTE/INTF/INTS: the RP2350 has two PWM interrupt lines; this driver uses IRQ0 only
        ...
        (0x0104 => irq1_inte: ReadWrite<u32, CH::Register>),
        (0x0108 => irq1_intf: ReadWrite<u32, CH::Register>),
        (0x010C => irq1_ints: ReadOnly<u32, CH::Register>),
        (0x0110 => @END),
    }
}
...
impl From<RPGpio> for ChannelNumber {
    // RP2350 datasheet, PWM "GPIO to slice mapping": GPIO 0-15 and 32-47 map to slices 0-7 (pairs),
    // GPIO 16-31 map to slices 8-11 (pairs, repeating every 8 GPIOs).
    fn from(gpio: RPGpio) -> Self {
        let g = gpio as u8;
        let slice = if (16..32).contains(&g) {
            8 + ((g - 16) >> 1 & 0b11)
        } else {
            g >> 1 & 0b111
        };
